...
async fn start_listener_for_downstream(
    downstream_addr: String,
    redirect_addr: String,
    server_priv_b64: String,
...
fn start_upstream_listener(upstream_addr: String, addr_stack: Arc<Mutex<Vec<(TcpStream, SocketAddr)>>>) {
    spawn(async move {
...
fn start_pinging(addr_stack: Arc<Mutex<Vec<(TcpStream, SocketAddr)>>>) {
    spawn(async move {
        loop {
